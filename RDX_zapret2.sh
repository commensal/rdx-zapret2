...
is_zapret2_running() {
    # Проверяем процессы zapret2: nfqws2 или tpws2
...
get_latest_version_zapret2() {
    ...
    # Если не получилось определить версию, используем запасной вариант
    echo "v1.0"
    return 1
}

download_zapret2_release() {
    local version="$1"
    local target_file="$2"

    debug "Пробуем скачать Zapret2 $version"

    # Пробуем разные возможные URL для zapret2
    local urls=(
        "https://github.com/bol-van/zapret2/releases/download/$version/zapret2-$version-openwrt-embedded.tar.gz"
        "https://github.com/bol-van/zapret2/releases/download/$version/zapret2-$version.tar.gz"
        "https://github.com/bol-van/zapret2/releases/download/$version/openwrt_embedded.zip"
        "https://github.com/bol-van/zapret2/archive/refs/tags/$version.tar.gz"
    )

    for url in "${urls[@]}"; do
        debug "Пробуем URL: $url"
        ...
        if curl -L -H "User-Agent: Mozilla/5.0" \
            ...
            if [ -f "$target_file" ]; then
                ...
                if [ "$size" -gt 100000 ]; then  # Минимум 100KB
...
install_zapret2_core() {
    ...
    if download_zapret2_release "$version" "$archive"; then
...
install_zapret2() {
    ...
    install_zapret2_core "$actual_path"
...
update_zapret2() {
    ...
    latest_version=$(get_latest_version_zapret2)
        ...
        case "$choice" in
            [Yy]*|"" )
                if [ "$TEST_MODE" = "true" ]; then
                    ...
                    if download_zapret2_release "$latest_version" "$archive"; then
                        print_success "Релиз Zapret2 скачан"
                        ...
                        if tar -xzf "$archive" -C "$temp_dir" 2>/dev/null; then
                            local found=false
                            for dir in "$temp_dir/zapret2/binaries/linux-arm" \
        ...
        if [ -d "$INSTALL_PATH" ] && [ -n "$(ls -A "$INSTALL_PATH" 2>/dev/null)" ]; then
            ...
            case "$choice" in
                1)
                    ...
                    ;;
                3) update_zapret2 ;;
                5) install_zapret2 "true" ;;
                6) full_uninstall_zapret2 ;;
                0|"")
                    echo ""
                    print_info "Выход..."
                    echo ""
