# rdX Zapret Installer 🚀

[![GitHub stars](https://img.shields.io/github/stars/commensal/rdx-zapret?style=social)](https://github.com/commensal/rdx-zapret)
[![GitHub forks](https://img.shields.io/github/forks/commensal/rdx-zapret?style=social)](https://github.com/commensal/rdx-zapret)

**Установщик оригинального [bol-van/zapret](https://github.com/bol-van/zapret) для рутованных "Dumb" роутеров Xiaomi rdX.**

## Поддерживаемые роутеры 📱

| Модель | 4PDA Тема |
|--------|-----------|
| [Xiaomi Router BE3600 2.5G](https://4pda.to/forum/index.php?showtopic=1082660) | [BE3600](https://4pda.to/forum/index.php?showtopic=1082660)  |
| [Xiaomi Router BE3600 Pro](https://4pda.to/forum/index.php?showtopic=1096055) | [BE3600 Pro](https://4pda.to/forum/index.php?showtopic=1096055)  |
| [Xiaomi Router BE5000 2.5G](https://4pda.to/forum/index.php?showtopic=1088946) | [BE5000](https://4pda.to/forum/index.php?showtopic=1088946) |

## Предварительные требования ⚠️
- **Root-доступ** уже получен ([xmir-patcher](https://github.com/openwrt-xiaomi/xmir-patcher)) 
- **Инструкция по руту НЕ включена** — читайте 4PDA
- **MobaXterm** ([скачать](https://mobaxterm.mobatek.net/download.html)) для SSH/SFTP редактирования

## 🚀 Установка одной командой

```bash
sh -c "$(curl -fsSL https://raw.githubusercontent.com/commensal/rdx-zapret/main/RDX_zapret.sh)"
```

## 🔧 Меню

```
1. (вкл/выкл) — Остановить/Запустить zapret
3. Проверить обновление
5. Принудительная переустановка
6. Полностью удалить zapret
0. Выйти (Enter)
```

## ⚙️ Конфигурация

**Все файлы в `/data/zapret/` (НЕ `/opt/zapret`!):**

### Основные файлы
- `/data/zapret/config` — параметры nfqws/tpws/mode
- `/data/zapret/ipset/zapret-hosts-user.txt` — список user доменов
- `/data/zapret/ipset/zapret-hosts-google.txt` — список google доменов

### Редактирование
```
MobaXterm → SSH root@192.168.31.1 → SFTP → /data/zapret/
```


## 🎯 Стратегии обхода
Дефолтные из данного репо ставятся автоматически и для кого-то работают для youtube, rutracker, nnmclub, telegram, whatsapp и тд. на МГТС Москва.
Могут работать, а могут и НЕ работать для Вас.

**Подбор вручную.** Источники:
- [remittor/zapret-openwrt #168](https://github.com/remittor/zapret-openwrt/discussions/168?sort=new)
- [StressOzz/Zapret-Manager](https://github.com/StressOzz/Zapret-Manager) (только идеи! Нельзя запускать на этих роутерах!)

**После смены стратегий или файлов ipset не забывайте перезапускать zapret (дважды опция 1 в скрипте).**

**Тест только на клиентах** — файрвол Xiaomi блокирует локальные проверки.

## ⚠️ Ответственность

- **Это только скрипт установщик** — Автор не несет ответсвенности за действия самого zapret
- **Zapret ≠ VPN** — Ничего не шифрует и не создает vpn тунелей, а только модифицирует пакеты на вашем роутере для тестирования способностей DPI вашего провайдера
- Скрипт **"как есть"**, бесплатно, без гарантий. Могут быть, а вероятнее всего точно **есть ошибки или рукозадость кода**.
- Автор не несет юридической, финансовой или моральной ответсвенности за ваши действия или действия этого скрипта в его неизменной или модифицированной форме.


## ☕ Спасибо
В случае благодарности автору и стимулирования доработки и улучшений  
-->❤️[Ссылка для добровольного пожертвования](https://vtb.paymo.ru/collect-money/?transaction=8841825f-2f16-4246-9d36-c45d64d868ec)❤️<--  
![Добровольное пожертвование](donate.jpg)

## 📄 Лицензия
MIT © 2026 commensal
```
```
